...
class _BookingScreenState extends State<BookingScreen> {
      ...
      if (isStart) {
        ...
        if (rentalType == 'day' && !endDate.isAfter(startDate)) {
          endDate = startDate.add(const Duration(days: 1));
            ...
            !_isEndTimeAfterStartTime(startTime, endTime, startDate, picked)) {
  ...
  Future<void> _pickTime({required bool isStart}) async {
    ...
    setState(() {
      if (isStart) {
        startTime = picked;
        if (rentalType == 'hour' &&
            !_isEndTimeAfterStartTime(picked, endTime, startDate, startDate)) {
  ...
  Widget build(BuildContext context) {
    ...
    // Calculate duration and total amount based on rental type
    int durationValue;
    int totalAmount;
    ...
    if (rentalType == 'hour') {
      ...
      durationValue = difference.inHours;
      totalAmount = durationValue * bike.pricePerHour;
      durationLabel = '$durationValue hour(s)';
    } else {
      durationValue = endDate.difference(startDate).inDays;
      totalAmount = durationValue * bike.pricePerDay;
      durationLabel = '$durationValue day(s)';
    }
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Expanded(
                    child: _RentalTypeButton(
                      label: 'Per Hour',
                      value: 'hour',
                      selected: rentalType == 'hour',
                      onSelected: (value) => setState(() => rentalType = value),
                    ),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: bike.isAvailable
                      ? () {
                          final startDateDisplay = rentalType == 'hour'
                              ? '${_formatDate(startDate)} ${_formatTime(startTime)}'
                              : _formatDate(startDate);
                          final endDateDisplay = rentalType == 'hour'
                              ? '${_formatDate(startDate)} ${_formatTime(endTime)}'
                              : _formatDate(endDate);
                          final startDateApi = rentalType == 'hour'
                              ? '${_formatApiDate(startDate)} ${_formatApiTime(startTime)}'
                              : _formatApiDate(startDate);
                          final endDateApi = rentalType == 'hour'
                              ? '${_formatApiDate(startDate)} ${_formatApiTime(endTime)}'
...
class _RentalTypeButton extends StatelessWidget {
  const _RentalTypeButton({
    required this.label,
    required this.value,
      ...
      child: Ink(
        ...
        child: Center(
          child: Text(
            label,
            style: TextStyle(
              color: selected ? inverseTextColor : secondaryColor,
              fontWeight: FontWeight.w600,
            ),
          ),
        ),
